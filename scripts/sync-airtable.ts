...
async function sync() {
  ...
  for (const record of records) {
    ...
    const dedicatedGfKitchenRaw = parseAiText(record.fields["dedicated_gf_kitchen"]);
    const dedicatedGfKitchen = dedicatedGfKitchenRaw ? dedicatedGfKitchenRaw.toLowerCase() : null;
