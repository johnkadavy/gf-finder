...
export default async function AccountPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="pt-16 max-w-6xl mx-auto px-4 md:px-8">
      ...
      >
        <div>
          <p className="font-mono text-ui-sm uppercase tracking-[0.3em] text-text-disabled mb-1">
            Account
          </p>
        ...
        >
          <div className="flex items-center justify-between gap-4 mb-5">
            ...
            <Link
              href={`/map/${shareToken}`}
              target="_blank"
              className="font-mono text-ui-sm uppercase tracking-editorial text-accent hover:underline shrink-0"
            >
              Preview →
            </Link>
          </div>
                ...
                >
                  <div className="min-w-0">
                    <Link
                      ...
                      className="group/name font-[family-name:var(--font-display)] leading-none truncate block hover:text-accent transition-colors duration-150"
                      style={{
                    ...
                    <p className="font-mono text-ui-md uppercase tracking-editorial text-text-label mt-2 truncate">
                      {[restaurant.neighborhood, restaurant.city].filter(Boolean).join(" / ")}
                    </p>
                    {restaurant.dossier?.summary?.short_summary && (
                      <p className="text-ui-xl leading-[1.7] text-text-secondary mt-2 max-w-xl">
                        {restaurant.dossier.summary.short_summary}
                      </p>
                    )}
                  ...
                  <div className="flex flex-col items-end shrink-0">
                    ...
                    <span
                      className="font-mono text-ui-sm uppercase tracking-label mt-1 text-right"
              ...
              return (
                <div key={visit.id} className="space-y-3">
                  ...
                  {restaurant && (
                    <Link
                      href={`/restaurant/${restaurant.slug ?? restaurant.id}`}
                      className="font-[family-name:var(--font-display)] text-[1.5rem] leading-none hover:text-accent transition-colors block"
