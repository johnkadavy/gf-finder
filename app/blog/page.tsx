  ...
  return (
    <main className="pt-16">
      ...
      <section className="px-4 md:px-8 py-10">
        <div className="max-w-4xl mx-auto">
          {posts.length === 0 ? (
            <p className="font-mono text-ui-md uppercase tracking-editorial text-text-disabled py-16 text-center">
              Posts coming soon
            </p>
          ) : (
            <div className="space-y-0">
              {posts.map((post) => (
                <Link
                  key={post.slug}
                  href={`/blog/${post.slug}`}
                  className="block border-b py-6 md:py-8 px-2 md:px-4 transition-colors duration-150 hover:bg-surface-raised"
                  style={{ borderColor: "var(--border-subtle)" }}
                >
                  <time
                    dateTime={post.date}
                    className="font-mono text-ui-sm uppercase tracking-editorial text-text-disabled block mb-2"
                  >
                    {formatDate(post.date)}
                  </time>
                  <h2
                    className="font-[family-name:var(--font-display)] leading-tight mb-2"
                    style={{ fontSize: "clamp(1.25rem, 3vw, 2rem)", letterSpacing: "0.02em", color: "var(--text-primary)" }}
                  >
                    {post.title}
                  </h2>
                  {post.description && (
                    <p className="text-ui-xl leading-[1.65] text-text-tertiary line-clamp-2 max-w-2xl">
                      {post.description}
                    </p>
                  )}
                  <span className="font-mono text-ui-sm uppercase tracking-editorial text-accent mt-3 inline-block">
                    Read →
                  </span>
                </Link>
              ))}
